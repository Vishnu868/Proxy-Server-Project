...
# Lists and cache for storing data received from the server
seperated_data = []
PROXY_CACHE_DATA = []
PROXY_CACHE_DATA_ID = []
...
while 1:
    ...
    try:
        ...
        # Extract opcode, indices, and data
        ...
        if opcode == "GET": # "OP=GET;IND=5,9,3;"
            for indice in indices_inp:
                if indice in PROXY_CACHE_DATA_ID:
                     # If data is in cache, retrieve and move to the front
            ...
            # Update the client message with the retrieved data
            ...
            for indice, data in zip(indices_inp, datas_inp) :
                if indice in PROXY_CACHE_DATA_ID:
                    # If data is in cache, update cache and send PUT request to server
                    location_of_element_in_list = PROXY_CACHE_DATA_ID.index(indice)
                    ...
                    # If data is not in cache, send PUT request to server and update cache
                    message_to_server = "PUT:" + indice + ":" + data + ":"
                    proxy_client_socket.sendall(bytes(message_to_server,'utf-8'))
                    time.sleep(0.5)
                    # Receive response from the server
            ...
            # Handle CLR operation (clear cache)
            PROXY_CACHE_DATA = []
            PROXY_CACHE_DATA_ID = []
