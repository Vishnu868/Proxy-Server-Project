...
#Loopback IP address
HOST = '127.0.0.1'   #test network connectivity on a local machine or on the same machine(127.0.0.0 to 127.255.255.255)
PORT = 6002
...
while 1:
    try:
        ...
        # Handle client disconnection, then wait for a new connection
        ...
        # Process different operations based on the opcode
        if (opcode == "GET"):
             # Retrieve data from the server data array at the specified index (ID)
            indice = int(dataReceived[1])
            dataReceived = int(SERVER_DATA[indice])
            print("Decoded OP: GET--> ID: ", indice, " DATA: ", dataReceived)
        
        elif (opcode == "PUT"):
            # Update the server data array at the specified index (ID) with the provided data
            indice = int(dataReceived[1])
            ...
            # Clear the entire server data array
            SERVER_DATA = [None] * 10
            dataReceived = "-"
            print("Decoded OP: CLR-->", " DATA: ", dataReceived)

        elif (opcode == "ADD"):
            # Similar to GET, retrieve data from the server data array at the specified index (ID)
            indice = int(dataReceived[1])
            dataReceived = int(SERVER_DATA[indice])
            print("Decoded OP: GET--> ID: ", indice, " DATA: ", dataReceived)

        # Display the current state of the server data array
